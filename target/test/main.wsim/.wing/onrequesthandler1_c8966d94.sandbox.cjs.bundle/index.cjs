...
// target/test/main.wsim/.wing/inflight.$Closure5-5.cjs
var require_inflight_Closure5_5 = __commonJS({
  "target/test/main.wsim/.wing/inflight.$Closure5-5.cjs"(exports2, module2) {
    "use strict";
    var $helpers = require_helpers();
    module2.exports = function({ $addTask, $std_Json }) {
      class $Closure5 {
        static {
        ...
        async handle(request) {
          ...
          if ($helpers.eq($helpers.lookup(tmp, "success"), true)) {
            ...
            })({ "message": $helpers.lookup(tmp, "message") }) };
          } else {
            return { "status": 504, "headers": { "Content-Type": "application/json" }, "body": ((json, opts) => {
              return JSON.stringify(json, null, opts?.indent);
            })({ "message": $helpers.lookup(tmp, "message") }) };
          }
        }
...
exports.handler = async function(event) {
  $handler = $handler ?? await (async () => {
    ...
    const $ctx = {
      handler: await (async () => {
        const $Closure5Client = require_inflight_Closure5_5()({
          $addTask: function() {
            let handle = process.env.FUNCTION_HANDLE_3a557aff;
            if (!handle) {
              throw new Error("Missing environment variable: FUNCTION_HANDLE_3a557aff");
            }
